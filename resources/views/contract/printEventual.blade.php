...
<body>
<div class="doc">
    <div class="head center"> CONTRATO EVENTUAL DE PRESTACIÓN DE SERVICIOS Nº {{ $contract->number_contract }} </div>
    <div class="content">
        <p class="text" style="text-indent: 5em">
            Conste por el presente “Contrato Eventual de Prestación de Servicios” que suscriben por una parte la <span class="title-text">MUTUAL DE SERVICIOS AL POLICIA - MUSERPOL</span>, representado legalmente por el <span class="title-text">Sr. Cnl. DESP. {{ Util::fullName($mae->employee) }} con C.I. No. {{ $mae->employee->identity_card.' '.$mae->employee->city_identity_card->shortened }}, en su calidad de {{ $mae->position->name }} de la MUSERPOL</span>, designado mediante Resolución Suprema Nº 16382 de fecha 31 de agosto de 2015, y por otra parte <span class="title-text up">{{ Util::fullName($contract->employee) }} con C.I. N° {{ $contract->employee->identity_card.' '.$contract->employee->city_identity_card->shortened }} </span>, mayor de edad y hábil por derecho, con domicilio en la ciudad de {{ $contract->employee->location }}, que en lo sucesivo se denominará el <span class="title-text">“CONTRATADO”</span> quienes celebran el presente contrato eventual, de acuerdo a los términos y condiciones siguientes:
        </p>
        <p>
        ...
        <p>
            <span class="title-text">CLÁUSULA TERCERA (PLAZO).</span> El plazo del presente contrato será a partir del {{ date('d', strtotime($contract->date_start)) }} de {{ Util::getMonthEs(date('m', strtotime($contract->date_start))) }} del {{ date('Y', strtotime($contract->date_start)) }} hasta el {{ date('d', strtotime($contract->date_end)) }} de {{ Util::getMonthEs(date('m', strtotime($contract->date_end))) }} de {{ date('Y', strtotime($contract->date_end)) }}, por tratarse de un contrato eventual, queda sobreentendido que el mismo quedará fenecido en la fecha señalada en la presente cláusula, salvando la previsión contenida en la Cláusula Décima del presente contrato.
        </p>
        <p>
        ...
        <p class="center">
            La Paz, {{ date('d', strtotime($contract->date_start)) }} de {{ Util::getMonthEs(date('m', strtotime($contract->date_start))) }} de {{ date('Y', strtotime($contract->date_start)) }}
        </p>

        <p class="firma center title-text">
            {{ $contract->employee->first_name.' '.$contract->employee->second_name.' '.$contract->employee->last_name.' '.$contract->employee->mothers_last_name }}<br>
            C.I. N° {{ $contract->employee->identity_card.' '.$contract->employee->city_identity_card->shortened }}
        </p>
    </div>
</div>    
</body>
